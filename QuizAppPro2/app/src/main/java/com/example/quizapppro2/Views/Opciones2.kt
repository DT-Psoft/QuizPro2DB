...
class Opciones2 : AppCompatActivity() {
    ...
    private val model by lazy { ViewModelProviders.of(this)[GameViewModel(
        this
    )::class.java] }
...
//        Numero de preguntas
        ...
        var adapter: ArrayAdapter<Int> =
            ArrayAdapter<Int>(
                ...
                Configuration.conf.arrayQuestionsNumber
            )
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_item)

        questionSpinner.adapter = adapter

        questionSpinner.setSelection(if(Configuration.conf.categoriesNumber != 1) Configuration.conf.questions_number - 5 else 0)

        questionSpinner.isEnabled = Configuration.conf.categoriesNumber != 1

        questionSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(p0: AdapterView<*>?, p1: View?, p2: Int, p3: Long) {
                Configuration.conf.questions_number =
...
//          Dificultad
        var idCurrentRadio = when (Configuration.conf.dificulty) {
        ...
        radioGroupDificultad.setOnCheckedChangeListener { _, _ ->
            when (radioGroupDificultad.checkedRadioButtonId) {
                altaRadioButton.id -> Configuration.conf.dificulty = 0
                mediaRadioButton.id -> Configuration.conf.dificulty = 1
                bajaRadioButton.id -> Configuration.conf.dificulty = 2
            }
        }
...
//          Numero de pistas
        var adapter1: ArrayAdapter<Int> =
            ArrayAdapter<Int>(
                this,
                android.R.layout.simple_spinner_item,
                Configuration.conf.arrayCluesNumber
            )
    ...
    private fun onCheckedChangeListener(pos: Int, checkBox: CheckBox) {
        if (Configuration.conf.categoriesNumber == 1 && !checkBox.isChecked){
            checkBox.isChecked = true

        } else {
            questionSpinner.isEnabled = true
            Configuration.conf.setEnabledCategory(pos, checkBox.isChecked)
            todosCheckBox.isChecked = Configuration.conf.categoriesNumber == 6
            if(Configuration.conf.categoriesNumber == 1){
    ...
    fun onRadioButtonClick(view: View) {
        ...
        var string: String = when (Configuration.conf.dificulty) {
            0 -> "Dificultad alta"
            1 -> "Dificultad media"
            2 -> "Dificultad baja"
            else -> ""
